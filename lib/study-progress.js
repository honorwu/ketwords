...
function isSpellingFrequencyEligible(frequencyScore, config = ensureStudyConfig()) {
  const score = Number(frequencyScore);
  const minimum = Number(config.spellFrequencyMinScore);

  return Number.isFinite(score) && Number.isFinite(minimum) && score >= minimum;
}

function isRepeatedWrong(state, config = ensureStudyConfig()) {
  return Number(state?.timesWrong || 0) >= Number(config.repeatedWrongThreshold);
}

function compareWordMapByFrequency(left, right) {
  const leftFrequency = Number(left?.frequencyScore ?? left?.frequencyZipf);
  const rightFrequency = Number(right?.frequencyScore ?? right?.frequencyZipf);
  const leftScore = Number.isFinite(leftFrequency) ? leftFrequency : Number.NEGATIVE_INFINITY;
  const rightScore = Number.isFinite(rightFrequency) ? rightFrequency : Number.NEGATIVE_INFINITY;
...
function getLearningTarget(row, config = ensureStudyConfig()) {
  const frequencyScore = row.frequency_score ?? row.frequencyScore;

  return isSpellingFrequencyEligible(frequencyScore, config) && hasSpellingText(row)
    ? "spell"
    : "listen";
}
...
function hydrateRow(row, config = ensureStudyConfig()) {
  ...
  return {
    ...
    frequencyZipf: Number.isFinite(row.frequency_zipf) ? row.frequency_zipf : null,
    childFrequencyZipf: Number.isFinite(row.child_frequency_zipf)
      ? row.child_frequency_zipf
      : null,
    frequencyScore: Number.isFinite(row.frequency_score) ? row.frequency_score : null,
    frequencySource: row.frequency_source || "unavailable",
