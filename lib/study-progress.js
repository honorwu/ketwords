...
function isSpellingFrequencyBand(frequencyBand, config = ensureStudyConfig()) {
  return (config.spellFrequencyBands || ["S"]).includes(frequencyBand);
}
...
function getLearningTarget(row, config = ensureStudyConfig()) {
  const frequencyBand = row.frequency_band || row.frequencyBand || "C";

  return isSpellingFrequencyBand(frequencyBand, config) && hasSpellingText(row)
    ? "spell"
    : "listen";
}
...
function hydrateRow(row, config = ensureStudyConfig()) {
  ...
  return {
    ...
    frequencyBand: row.frequency_band || "C",
