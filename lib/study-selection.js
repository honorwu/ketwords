...
function sortStudyCandidates(left, right, mode) {
  const useSpellingPriority = mode === "spell";
  const leftScore = useSpellingPriority
    ? PRIORITY_SCORE[left.priority] || 1
    : FREQUENCY_BAND_SCORE[left.frequencyBand] || 1;
  const rightScore = useSpellingPriority
    ? PRIORITY_SCORE[right.priority] || 1
    : FREQUENCY_BAND_SCORE[right.frequencyBand] || 1;
  ...
  return (
    ...
    (!useSpellingPriority ? rightFrequency - leftFrequency : 0) ||
    leftDue - rightDue ||
    leftRank - rightRank ||
