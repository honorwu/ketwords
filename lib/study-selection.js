...
function getDeferredSpellRetryCandidates(
  states,
  now,
  todaySpellAttemptCounts = new Map(),
  maxAttemptsPerDay = 2
) {
  return states
    .filter((state) => {
      ...
      return (todaySpellAttemptCounts.get(state.normalizedTerm) || 0) < maxAttemptsPerDay;
    })
    .sort((left, right) => sortStudyCandidates(left, right, "spell"));
}

function getNextReviewAtAfterAnswer(mode, result, masteredAfter, now = new Date()) {
  if (masteredAfter) {
    return addDays(now, 3650);
  }

  if (result === "wrong" && (mode === "recognize" || mode === "listen")) {
