...
const DAILY_RECOGNIZE_TARGET = 30;
...
function createStore() {
  ...
  db.exec(`
    ...
    SET listening_stage = 0,
        next_review_at = COALESCE(last_seen_at, first_seen_at, next_review_at, datetime('now')),
        correct_streak = 0
    WHERE listening_stage > 0
      AND COALESCE(word_key, '') IN (
        SELECT normalized_term
        FROM ${wordsTable}
        WHERE learning_target = 'spell'
      )
      AND NOT EXISTS (
        SELECT 1
        FROM study_logs
        WHERE study_logs.mode = 'listen'
          AND (
            study_logs.word_key = progress.word_key
            OR (study_logs.word_key IS NULL AND study_logs.word_id = progress.word_id)
          )
      );
  `);
  ...
  function getDailyActivity(days = 120, monthOffset = 0) {
    ...
    const start = addDays(currentMonthStart, -Math.max(days - 30, 0));
    const startKey = todayKey(start);
    const rows = db
      .prepare(
        `
  ...
  function sortStudyCandidates(left, right) {
