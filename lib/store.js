...
function ensureWordBankSchema() {
  ...
  try {
    ...
    addColumnIfMissing(wordBankDb, "words", "frequency_band TEXT");
...
function createStore() {
  ...
  const progressJoin = `
    p.word_key = w.normalized_term
    OR (
      p.word_id = w.id
      AND NOT EXISTS (
        SELECT 1
        FROM ${wordsTable} keyed_word
        WHERE keyed_word.normalized_term = p.word_key
      )
    )
  `;
  ...
  function getTodayWrongWords() {
    ...
    return db
      ...
      .map((row) => ({
        ...
        frequencyZipf: row.frequency_zipf,
        ...
        remainingRecognize > 0
          ...
          : remainingListen > 0
            ...
            : remainingSpell > 0
              ...
              : "\u4eca\u5929\u7684\u4e09\u6bb5\u76ee\u6807\u5df2\u5b8c\u6210\uff0c\u63a5\u4e0b\u6765\u6309 6 \u4e2a\u8ba4\u8bcd\u30012 \u4e2a\u542c\u8bcd\u30011 \u4e2a\u62fc\u5199\u7684\u8282\u594f\u52a0\u7ec3\u3002",
      adaptiveNote:
  ...
  function getWordProgress() {
    return getAllStates()
      .sort((left, right) => {
        ...
        const leftFrequencyBand = FREQUENCY_BAND_SCORE[left.frequencyBand] || 1;
        const rightFrequencyBand = FREQUENCY_BAND_SCORE[right.frequencyBand] || 1;
        ...
        return (
          ...
          rightFrequencyBand - leftFrequencyBand ||
