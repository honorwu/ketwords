...
const MAX_DAILY_SPELL_ATTEMPTS = 2;
...
function createStore() {
  ...
  db.exec(`
    ...
    WHERE last_result = 'wrong'
      AND last_mode IN ('recognize', 'listen')
      AND last_seen_at IS NOT NULL;

    UPDATE progress
    SET next_review_at = last_seen_at
    WHERE last_result = 'wrong'
      AND last_mode = 'spell'
      AND last_seen_at IS NOT NULL;
  ...
  function masteryLabel(state) {
    const statusLabels = {
      ...
      recognize: DISPLAY_LABELS.statusRecognize,
      listen: DISPLAY_LABELS.statusListen,
      spell: DISPLAY_LABELS.statusSpell,
      "spell-retry": DISPLAY_LABELS.statusSpellRetry,
  ...
  function getTodaySpellAttemptCounts() {
    const rows = db
      .prepare(
        `
          SELECT word_key, COUNT(*) AS attempt_count
          FROM study_logs
          WHERE studied_on = ?
            AND mode = 'spell'
          GROUP BY word_key
    ...
    return new Map(
      rows
        .filter((row) => row.word_key)
        .map((row) => [row.word_key, Number(row.attempt_count) || 0])
    );
  }
  ...
  function assertAnswerModeAllowed(state, mode) {
    ...
    if (!isAvailableForModeToday(state, mode, todayModeWordKeys)) {
      const isAllowedSpellRetry =
        mode === "spell" &&
        state.lastMode === "spell" &&
        state.lastResult === "wrong" &&
        (getTodaySpellAttemptCounts().get(state.normalizedTerm) || 0) <
          MAX_DAILY_SPELL_ATTEMPTS;

      if (!isAllowedSpellRetry) {
        throw submissionBlocked("这个词今天已经完成当前环节，请换一个词。");
      }
    }
  }
    ...
    return getAllStates()
      ...
      .map((state) => {
        ...
        if (isMastered(state)) {
          ...
          nextAction = DISPLAY_LABELS.tomorrowListen;
        } else if (mapStatus === "spell-retry") {
  ...
  function submitAnswer(payload) {
    ...
    const nextReviewAt = getNextReviewAtAfterAnswer(
      payload.mode,
      evaluation.result,
      masteredAfter,
      now
  ...
  function getTextDiagnostics() {
    return {
      started: DISPLAY_LABELS.statusRecognize,
