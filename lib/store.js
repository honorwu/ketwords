...
function createStore(dbPath = DB_PATH) {
  ...
  function computeStudyPlan(states) {
    ...
    adjustedTarget = clamp(adjustedTarget, 5, 32);
  ...
  function buildOverview() {
    ...
    const coreGoalCount = states.filter(isCoreGoal).length;
    const coreMastered = states.filter(
      (state) => isCoreGoal(state) && isMastered(state)
    ).length;
    const spellGoalCount = states.filter(
      (state) => state.learningTarget === "spell"
    ).length;
    const spellMastered = states.filter(
      (state) => state.learningTarget === "spell" && isMastered(state)
    ).length;
    const listenGoalCount = states.filter(
      (state) => state.learningTarget === "listen"
    ).length;
    const listenMastered = states.filter(
      (state) => state.learningTarget === "listen" && isMastered(state)
    ).length;
    const recognizeGoalCount = states.filter(
      (state) => state.learningTarget === "recognize"
    ).length;
    const recognizeMastered = states.filter(
      (state) => state.learningTarget === "recognize" && isMastered(state)
    ).length;
    ...
    return {
      ...
      parentMessage:
        projectedCoreByExam >= coreGoalCount
          ? "按当前速度，预计能在考试前完成核心目标。"
          : `按当前速度，预计考试前可完成约 ${projectedPercent}% 的核心目标。`,
    };
  }
  ...
  function submitAnswer(payload) {
    ...
    if (payload.mode === "spell") {
      ...
      evaluation = {
        result: payload.choiceWordId === payload.wordId ? "correct" : "wrong",
        acceptedText: state.term,
        note:
          payload.choiceWordId === payload.wordId
            ? "回答正确。"
            : `正确答案是 ${state.chineseMeaning || state.term}。`,
      };
    }
    ...
    `).run(
      ...
      payload.mode,
      evaluation.result,
      payload.mode === "spell" ? payload.response || "" : String(payload.choiceWordId || ""),
