...
// These groups only prevent reviewed word-bank entries from appearing together.
// Option labels still come directly from each word's stored Chinese meaning.
const DISTRACTOR_COLLISION_GROUPS = [
  ["a", "an", "one"],
  ["a few", "few", "several"],
  ["all kinds of", "all sorts of", "various"],
  ["around", "could", "maybe", "might", "ok", "perhaps", "possible", "possibly", "probably"],
  ["beautiful", "fine", "good-looking", "lovely", "pretty", "smart"],
  ["career", "job", "occupation", "weekday", "work", "working hours"],
  ["say", "speak", "talk"],
  ["afraid", "frightened", "nervous", "scared", "worried", "worry"],
  ["all right", "excellent", "good", "nice", "ok", "perfect", "well", "wonderful"],
  ["awful", "bad", "horrible", "scary", "terrible", "worse", "worst"],
  ["big", "huge", "large"],
  ["little", "small"],
  ["fast", "quick", "quickly"],
  ["easily", "easy", "simple"],
  ["difficult", "hard"],
  ["begin", "start"],
  ["complete", "end", "finish", "stop"],
  ["choose", "decide"],
  ["actually", "real", "really", "true"],
  ["close to", "near", "next to"],
  ["computer", "laptop", "notebook", "pc"],
  ["bus station", "railway", "station"],
  ["railway", "train"],
  ["friday", "monday", "saturday", "sunday", "thursday", "tuesday", "wednesday", "week"],
  ["bring back", "come back", "get back"],
  ["cash", "money"],
  ["fix", "repair"],
  ["group", "team"],
  ["photo", "photographer", "photography", "picture"],
  ["hear", "listen"],
  ["discover", "find", "find out", "look for"],
  ["closed", "close", "shut", "turn off"],
  ["open", "turn on"],
  ["empty", "free", "spare"],
  ["glad", "happy", "pleased"],
  ["sad", "unhappy", "upset"],
  ["cell phone", "mobile", "phone"],
  ["aeroplane", "airport", "plane"],
  ["animal", "zoo"],
  ["class", "student"],
  ["grandad", "grandfather", "grandma", "grandmother", "grandpa", "grandparent", "granny"],
  ["concert", "music", "musical", "musician", "pop", "tune"],
  ["football player", "footballer", "soccer", "sport", "tennis player"],
  ["good afternoon", "p.m."],
  ["have got to", "have to", "must", "should"],
  ["i", "me", "mine", "my", "myself", "own"],
  ["he", "him", "himself", "his", "own"],
  ["her", "hers", "herself", "own", "she"],
  ["ours", "our", "ourselves", "own", "us", "we"],
  ["own", "their", "theirs", "them", "themselves", "they"],
  ["own", "you", "your", "yours", "yourself"],
  ["cd", "cd player", "dvd", "dvd player"],
  ["amazed", "amazing"],
  ["bored", "boring"],
  ["excited", "exciting"],
  ...
  ["shall", "will", "would"],
  ["give somebody a call", "telephone"],
  ["serve", "waiter", "waitress"],
  ["across", "dressed", "through", "wear"],
  ["ice skating", "skate", "skating"],
  ["above", "than"],
  ["all", "total"],
  ["away", "get off", "leave", "off"],
  ...
  ["doll", "toy"],
  ["each", "every", "everybody", "everyone", "per"],
  ["enter", "into"],
  ["file", "folder"],
  ["finally", "last"],
  ["for sale", "sell"],
  ["get fit", "health", "healthy"],
  ["include", "including"],
  ["information", "news"],
  ["invitation", "invite"],
  ["look at", "see"],
  ...
  ["strange", "surprising"],
  ["striped", "stripes"],
  ["surf", "surfboard", "surfboarding", "surfing", "windsurfing"],
  ["thank you", "thanks"],
  ["walk", "walking"],
  ["certainly", "of course", "sure"],
  ["doctor", "dr"],
  ["mistake", "wrong"],
  ["amazed", "oh dear!", "surprised"],
  ["at", "sign"],
  ["be", "yes"],
  ["belong", "hers", "of", "ours"],
  ["careful", "carefully", "look out", "safe"],
  ["digital", "number"],
  ...
  ["time", "working hours"],
  ["together", "with"],
  ["tourist", "tourist information centre", "visitor"],
];

const DISTRACTOR_COLLISION_LOOKUP = new Map();

for (const [groupIndex, group] of DISTRACTOR_COLLISION_GROUPS.entries()) {
  for (const term of group) {
    const key = normalizeDistractorTerm(term);

    if (!DISTRACTOR_COLLISION_LOOKUP.has(key)) {
      DISTRACTOR_COLLISION_LOOKUP.set(key, new Set());
    }

    DISTRACTOR_COLLISION_LOOKUP.get(key).add(groupIndex);
  }
}

function getDistractorTermKeys(state) {
  return Array.from(
    new Set(
      [state?.term, state?.baseTerm]
...
function shareDistractorCollisionGroup(left, right) {
  const leftGroups = new Set();

  for (const term of getDistractorTermKeys(left)) {
    for (const groupIndex of DISTRACTOR_COLLISION_LOOKUP.get(term) || []) {
      leftGroups.add(groupIndex);
    }
  }

  for (const term of getDistractorTermKeys(right)) {
    for (const groupIndex of DISTRACTOR_COLLISION_LOOKUP.get(term) || []) {
      if (leftGroups.has(groupIndex)) {
        return true;
      }
    }
  }
...
function termsAndMeaningsAreNested(left, right) {
  const leftMeaning = normalizeMeaning(left?.chineseMeaning).replace(/[的地得]/g, "");
  const rightMeaning = normalizeMeaning(right?.chineseMeaning).replace(/[的地得]/g, "");

  if (
    !leftMeaning ||
    !rightMeaning ||
    (!leftMeaning.includes(rightMeaning) && !rightMeaning.includes(leftMeaning))
  ) {
    return false;
  }

  for (const leftTerm of getDistractorTermKeys(left)) {
    const compactLeft = leftTerm.replace(/\s+/g, "");

    for (const rightTerm of getDistractorTermKeys(right)) {
      const compactRight = rightTerm.replace(/\s+/g, "");

      if (
        Math.min(compactLeft.length, compactRight.length) >= 2 &&
...
function areDistractorWordsTooClose(left, right) {
  const leftTerms = getDistractorTermKeys(left);
  const rightTerms = new Set(getDistractorTermKeys(right));

  if (leftTerms.some((term) => rightTerms.has(term))) {
    return true;
  }

  return (
    shareDistractorCollisionGroup(left, right) ||
    termsAndMeaningsAreNested(left, right)
  );
}
...
function selectRandomDistractors(candidate, states, limit = 16, random = Math.random) {
  ...
  const candidates = states.filter((state) => {
    return (
      ...
      !areDistractorWordsTooClose(candidate, state)
    );
  });
  ...
  for (const state of candidates) {
    ...
    if (
      ...
      selected.some((item) => areDistractorWordsTooClose(item, state))
    ) {
      continue;
    }
...
function createStore() {
  ...
  db.exec(`
    ...
    UPDATE progress
    SET recognition_stage = CASE WHEN recognition_stage > 0 THEN 1 ELSE 0 END,
        listening_stage = CASE WHEN listening_stage > 0 THEN 1 ELSE 0 END,
        spelling_stage = CASE WHEN spelling_stage > 0 THEN 1 ELSE 0 END;

    UPDATE progress
    SET recognition_stage = 0
    WHERE last_mode = 'recognize'
      AND last_result = 'wrong';

    UPDATE progress
    SET recognition_stage = 0,
        listening_stage = 0,
        spelling_stage = 0
    WHERE last_mode = 'listen'
      AND last_result = 'wrong';

    UPDATE progress
    SET spelling_stage = 0
    WHERE last_mode = 'spell'
      AND last_result = 'wrong';

    UPDATE progress
    SET next_review_at = strftime(
      '%Y-%m-%dT%H:%M:%fZ',
      last_seen_at,
      'localtime',
      'start of day',
    ...
    WHERE last_result = 'wrong'
      AND last_mode IN ('recognize', 'listen', 'spell')
    ...
    WHERE last_result = 'correct'
      AND last_seen_at IS NOT NULL
      AND (
        next_review_at IS NULL
        OR julianday(next_review_at) <= julianday(last_seen_at, '+10 minutes')
      );
  ...
  function masteryLabel(state) {
    const statusLabels = {
      unseen: DISPLAY_LABELS.statusUnseen,
      "recognize-wrong": DISPLAY_LABELS.statusRecognizeWrong,
      "listen-pending": DISPLAY_LABELS.statusListenPending,
      "listen-wrong": DISPLAY_LABELS.statusListenWrong,
      "spell-pending": DISPLAY_LABELS.statusSpellPending,
      "spell-wrong": DISPLAY_LABELS.statusSpellWrong,
      mastered: DISPLAY_LABELS.mastered,
    };

    return statusLabels[getWordMapStatus(state)] || DISPLAY_LABELS.statusUnseen;
  }
  ...
  function getTodayStudiedWordKeysByMode() {
    ...
    for (const row of rows) {
      if (sets[row.mode] && row.word_key) {
        sets[row.mode].add(getDailyWordKey(row));
      }
    }
  ...
  function getWrongWordRankings(limitPerMode = 6) {
    const rankings = {
      recognize: [],
      listen: [],
    };
      ...
      .prepare(
        `
          SELECT
            ...
            MIN(w.id) AS word_id,
            MIN(w.term) AS term,
            GROUP_CONCAT(DISTINCT w.chinese_meaning) AS meanings,
            COUNT(*) AS attempt_count,
            SUM(CASE WHEN l.result = 'wrong' THEN 1 ELSE 0 END) AS wrong_count,
            MAX(CASE WHEN l.result = 'wrong' THEN l.created_at END) AS last_wrong_at
          FROM study_logs l
          INNER JOIN ${wordsTable} w
            ON w.normalized_term = l.word_key
          WHERE l.mode IN ('recognize', 'listen')
          GROUP BY l.mode, lower(trim(COALESCE(w.base_term, w.term, l.word_key)))
          HAVING SUM(CASE WHEN l.result = 'wrong' THEN 1 ELSE 0 END) > 0
          ORDER BY
            l.mode ASC,
            wrong_count DESC,
            (wrong_count * 1.0 / attempt_count) DESC,
            last_wrong_at DESC,
            term ASC
        `
      )
    ...
    for (const row of rows) {
      if (!rankings[row.mode] || rankings[row.mode].length >= limitPerMode) {
        continue;
      }
      ...
      rankings[row.mode].push({
        ...
        wrongCount,
        attemptCount,
        errorRate: attemptCount > 0 ? Math.round((wrongCount / attemptCount) * 100) : 0,
  ...
  function buildOverview() {
    ...
    return {
      ...
      hardWords,
      wrongWordRankings: getWrongWordRankings(),
  ...
  function assertAnswerModeAllowed(state, mode) {
    if (isMastered(state)) {
      throw submissionBlocked("这个词已经掌握，不需要重复作答。");
    }

    const expectedMode = getModeForState(state);

    if (mode !== expectedMode) {
      throw submissionBlocked(`这个词当前应进行${getModeLabel(expectedMode)}。`);
    }

    const todayModeWordKeys = getTodayStudiedWordKeysByMode();

    if (!isWordAvailableToday(state, todayModeWordKeys)) {
      throw submissionBlocked("这个词今天已经出现过了，请换一个词。");
    }
  }
  ...
  function getNextCandidate() {
    ...
    for (const mode of getModePriorityOrder()) {
    ...
    const deferredSpellCandidates = getDeferredSpellRetryCandidates(
      states,
      now,
      todayModeWordKeys
    );

    if (deferredSpellCandidates.length > 0) {
      return {
        status: "ready",
        candidate: deferredSpellCandidates[0],
        mode: "spell",
  ...
  function getWordProgress() {
    ...
    return getAllStates()
      ...
      .map((state) => {
        const mapStatus = getWordMapStatus(state);
        const deferredMode = getDeferredMode(state);
        let nextAction = getModeLabel(getModeForState(state));

        if (isMastered(state)) {
          nextAction = DISPLAY_LABELS.longReview;
        } else if (deferredMode === "recognize") {
          nextAction = DISPLAY_LABELS.tomorrowRecognize;
        } else if (deferredMode === "listen") {
          nextAction = mapStatus === "listen-wrong"
            ? DISPLAY_LABELS.tomorrowListenRetry
            : DISPLAY_LABELS.tomorrowListen;
        } else if (mapStatus === "spell-wrong") {
          nextAction = DISPLAY_LABELS.spellRetry;
        } else if (deferredMode === "spell") {
          nextAction = DISPLAY_LABELS.tomorrowSpell;
        }
  ...
  function submitAnswer(payload) {
    ...
    const nextReviewAt = getNextReviewAtAfterAnswer(masteredAfter, now);
  ...
  function getTextDiagnostics() {
    return {
      started: DISPLAY_LABELS.statusRecognizeWrong,
