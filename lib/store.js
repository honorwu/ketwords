...
function createStore() {
  ...
  const parentFocusJoin = "f.word_key = w.normalized_term OR (f.word_key IS NULL AND f.word_id = w.id)";
  ...
  function evaluateSpelling(state, response) {
    ...
    const nearMatch = accepted.find((value) => {
      if (!compactResponse || !value.compact) {
        return false;
      }

      const maxLength = Math.max(compactResponse.length, value.compact.length);
      const allowedDistance = maxLength <= 5 ? 1 : 2;
      return levenshtein(compactResponse, value.compact) <= allowedDistance;
    });

    if (nearMatch) {
      return {
        result: "almost",
        acceptedText: nearMatch.raw || state.baseTerm,
        note: `差一点点，正确写法是 ${nearMatch.raw || state.baseTerm}。`,
      };
    }
