...
function isNonSpellingParenthetical(value) {
  ...
  if (NON_SPELLING_PARENTHETICALS.has(label)) {
    return true;
  }
...
function buildSpellingCandidates(rawTerm) {
  ...
  for (const candidate of candidates) {
    ...
    if (optionalWordMatch) {
      ...
      const optional = optionalWordMatch[2].trim();
      expanded.push(prefix, `${prefix} ${optional}`);
      continue;
    }
  ...
  return expanded
    .map((term) => stripTrailingNonSpellingNotes(term).trim())
    .filter(Boolean);
}
...
function classifyWord(term, pos) {
  ...
  if (grammarHeavy) {
    ...
    priority = "S";
  } else if (isListen || theme !== "general") {
    priority = "A";
  } else if (
    term.includes(" ") ||
