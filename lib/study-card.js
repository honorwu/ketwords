...
function getSingleWordSpellings(acceptedSpellings, fallbackTerm) {
  const spellings = (Array.isArray(acceptedSpellings) ? acceptedSpellings : [])
    .map((term) => String(term || "").trim())
    .filter((term) => term && !/\s/.test(term));
...
function getSpellingLengths(acceptedSpellings, fallbackTerm) {
  const lengths = getSingleWordSpellings(acceptedSpellings, fallbackTerm)
...
function createCardBuilder(store) {
  ...
  return async function buildCard() {
    ...
    const pool = store.getDistractorPool(candidate.wordId, 40);
    const distractorCandidates = await Promise.all(
      pool.slice(0, 24).map((item) => ensureWordMeaning(item))
    );
    ...
    for (const item of distractorCandidates) {
      ...
      if (areOptionLabelsTooClose(candidateLabel, label)) {
        continue;
      }
