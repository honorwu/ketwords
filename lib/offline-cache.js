...
const { cleanChineseMeaning } = require("./word-text");
...
const TRANSLATION_OVERRIDES = new Map([
  ["all right", "好的；没关系"],
  ["barbecue", "烧烤"],
  ["cafe", "咖啡馆"],
  ["café", "咖啡馆"],
...
const ENTRY_TRANSLATION_OVERRIDES = new Map([
  ["a/an", "一；一个"],
  ["a an", "一；一个"],
  ["all right/alright", "好的；没关系"],
  ["all right alright", "好的；没关系"],
  ["at / @", "在；@ 符号"],
  ["at @", "在；@ 符号"],
  ...
  ["id card", "身份证；身份卡"],
  ["it", "信息技术"],
  ["jewellery n br eng", "珠宝；首饰"],
  ["kilometre km n br eng", "千米；公里"],
  ["litre n br eng", "升"],
  ["lots / a lot", "许多；大量"],
  ["lots a lot", "许多；大量"],
  ["metre n br eng", "米"],
  ...
  ["poor thing/you", "可怜的人；可怜的你"],
  ["poor thing you", "可怜的人；可怜的你"],
  ["practice n br eng", "练习；实践"],
...
function cleanTranslationPayload(payload) {
  ...
  return cleanChineseMeaning(payload[0]
    .map((part) => part[0] || "")
    .join("")
    .replace(/\s+/g, " ")
    .trim());
...
function getEntryTranslationOverride(state) {
  const keys = [
  ...
  for (const key of keys) {
    ...
    if (ENTRY_TRANSLATION_OVERRIDES.has(normalizedKey)) {
      return cleanChineseMeaning(ENTRY_TRANSLATION_OVERRIDES.get(normalizedKey));
    }
  }
...
async function translateToChinese(term) {
  ...
  const override = TRANSLATION_OVERRIDES.get(lookupTerm.toLowerCase());

  if (override) {
    return cleanChineseMeaning(override);
  }

  const url =
    "https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=t&q=" +
    encodeURIComponent(lookupTerm);
...
function getComputedAudioUrls(state) {
  const mp3Url = `/audio/${getComputedAudioFileName(state)}`;
  return [mp3Url, mp3Url.replace(/\.mp3$/, ".m4a")];
}
...
function getExistingLocalAudioUrl(state) {
  ...
  for (const computedUrl of getComputedAudioUrls(state)) {
    if (fileExistsAndHasContent(publicUrlToFilePath(computedUrl))) {
      return computedUrl;
    }
  }
...
async function ensureWordOfflineData(store, state, options = {}) {
  ...
  if (settings.includeMeaning && !current.chineseMeaning) {
    const chineseMeaning =
      getEntryTranslationOverride(current) ||
      (settings.allowNetwork ? await translateToChinese(pickLookupTerm(current)) : "");
