...
系统只使用一套数值词频：`frequency_score` 决定同一训练阶段内的单词顺序，不再使用 S/A/B/C 档位。考试日期和错题等待时间集中保存在 `data/study-config.json`：
...
系统不再设置每天认词、听词或拼写的数量。选题只使用固定优先级 `认词 > 听词 > 拼写`：只要还有可做的认词，就先做认词；没有可做的认词才进入听词；认词和听词都没有可做内容时才进入拼写。同一个英文显示词形一天最多出现一次，即使词库中有多个词性或义项，也不会在当天连续出现；当天做过认词的词不会再进入听词或拼写。答对后只更新到下一状态，最早第二天进入下一环节。
...
- `待听词`：认词已经答对，还没有做过听词。
- `听词错误`：做过听词，但听词答错。
- `待拼写`：认词和听词都已答对，还没有开始拼写。
- `拼写错误`：做过拼写，但拼写答错。
- `已掌握`：单个单词拼写通过；不需要拼写的词组则在听词通过后掌握。

认词答错后，第二天继续认词；听词答错后会清空认词通过状态，第二天从认词重新开始。答对后最早第二天进入下一环节。拼写错误当天不再出现，从第二天起只有普通任务清空后才会回到队尾。纯字母或数字的单个词需要拼写；包含空格、短划线、句点等符号的词组不进入拼写。

家长后台分别显示“认词错误榜”和“听词错误榜”，按累计错误次数排序，同时显示错误次数、答题次数和错误率。同一英文显示词形的不同词性或义项会合并统计。

认词和听词的中文选择题只使用词库中已有的固定中文释义：正确项使用当前单词释义，另外 3 项从相同主题类别的其他单词中随机抽取；系统不会为选项临时生成或改写释义。

随机抽取前会排除英文显示相同、英文与中文同时互相包含，以及词库排查中已确认容易混淆的词组；四个选项之间会两两检查，避免干扰项彼此过近。

家长掌握地图按 `frequency_score` 从左上到右下递减排列，用底色表示上述 7 个主状态。累计错误达到 `repeatedWrongThreshold` 的词会额外显示“反复错”红点，并可单独筛选；它是历史标签，可以与任意主状态同时存在，不会覆盖当前状态。方格默认不显示文字，悬停可看单词，点击可看完整学习详情。
