...
完成每日 `60/20/10` 后，系统按 `6 个认词、2 个听词、1 个拼写` 的节奏继续加练。答对后状态会立即进入下一环节，因此同一个词可以在同一天先后完成认词、听词和拼写。
...
拼写答错会记录为“拼写待重试”，并从普通队列中移走。普通任务清空后，这些错词才会回到队尾；为避免只剩一个错词时无限循环，同一个词每天最多进行两次拼写。
