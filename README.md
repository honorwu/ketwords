...
系统不再设置每天认词、听词或拼写的数量。选题只使用固定优先级 `认词 > 听词 > 拼写`：只要还有可做的认词，就先做认词；没有可做的认词才进入听词；认词和听词都没有可做内容时才进入拼写。同一个词一天最多出现一次，无论当天做的是认词、听词还是拼写；答对后只更新到下一状态，最早第二天进入下一环节。

学习流程固定为 `未学习 → 认词中 → 听词中 → 拼写中 → 已掌握`。认词或听词答错后保留在当前环节，第二天优先继续该环节；答对后进入下一环节。纯字母或数字的单个词都需要拼写一次，拼对后标记为已掌握。包含空格、短划线、句点等符号的词组不进入拼写，听对后直接标记为已掌握。

拼写答错会记录为“拼写待重试”，当天不再出现，并从普通队列中移走。从第二天起，只有普通任务清空后，这些错词才会回到队尾。
...
家长掌握地图按 `frequency_score` 从左上到右下递减排列，用底色表示词语当前所处阶段：未学习、认词中、听词中、拼写中、拼写待重试或已掌握。累计错误达到 `repeatedWrongThreshold` 的词会额外显示红点，并可单独筛选；红点表示历史上反复出错，不会覆盖当前掌握阶段。方格默认不显示文字，悬停可看单词，点击可看完整学习详情。
