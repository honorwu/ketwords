...
系统只使用一套数值词频：`frequency_score` 决定认词、听词和拼写顺序，不再使用 S/A/B/C 档位。考试日期、每日目标、加练节奏和错题等待时间都集中保存在 `data/study-config.json`：
...
当天三个训练批次的排程顺序为 `认词 > 听词 > 拼写`：先完成每日 `60/20/10`，之后按 `6 个认词、2 个听词、1 个拼写` 的节奏继续加练。同一个词一天最多出现一次，无论当天做的是认词、听词还是拼写；答对后只更新到下一状态，最早第二天进入下一环节。
